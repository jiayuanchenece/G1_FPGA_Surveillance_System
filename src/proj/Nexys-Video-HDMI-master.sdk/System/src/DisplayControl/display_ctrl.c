...
int DisplayStart(DisplayCtrl *dispPtr)
{
    ...
    */
	...
	Status = XAxiVdma_DmaConfig(dispPtr->overlayvdma, XAXIVDMA_READ, &(dispPtr->vdmaConfig));
	if (Status != XST_SUCCESS)
	{
		xil_printf("Read channel config failed %d\r\n", Status);
		return XST_FAILURE;
	...
	Status = XAxiVdma_DmaSetBufferAddr(dispPtr->overlayvdma, XAXIVDMA_READ, dispPtr->vdmaConfig.FrameStoreStartAddr);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Read channel set buffer address failed %d\r\n", Status);
		return XST_FAILURE;
	...
	Status = XAxiVdma_DmaStart(dispPtr->overlayvdma, XAXIVDMA_READ);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Start read transfer failed %d\r\n", Status);
		return XST_FAILURE;
	...
	Status = XAxiVdma_StartParking(dispPtr->overlayvdma, ((dispPtr->curFrame) + 1) % 3, XAXIVDMA_READ);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Unable to park the channel %d\r\n", Status);
		return XST_FAILURE;
...
**
*/
int DisplayInitialize(DisplayCtrl *dispPtr, XAxiVdma *vdma, XAxiVdma *overlayvdma,u16 vtcId, u32 dynClkAddr, u8 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
{
	int Status;
	int i;
    ...
    */
	...
	dispPtr->vdma = vdma;
	dispPtr->overlayvdma = overlayvdma;
